...
constexpr auto build_tuple(const string& line, char delimiter, size_t start, int_<N>) {
  ...
  return tuple_cat(
    ...
    build_tuple<Tuple>(line, delimiter, found + 1, int_<N-1>())
  );
}

template <class Tuple>
constexpr auto build_tuple(const string& line, char delimiter, size_t start, int_<1>) {
  typedef typename tuple_element<tuple_size<Tuple>::value-1, Tuple>::type FieldTp;
  size_t found = line.find(delimiter, start);
  return make_tuple(convert<FieldTp>(line.substr(start, found - start)));
}
...
template <class Tuple>
constexpr void fill_tuple(Tuple& tuple, const string& line, char delimiter) {
  return fill_tuple(tuple, line, delimiter, 0, int_<tuple_size<Tuple>::value>());
}
