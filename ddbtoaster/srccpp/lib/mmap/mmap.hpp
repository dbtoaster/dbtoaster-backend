    ...
    inline virtual void slice(const T& key, std::function<void (const T&) > f) {
        ...
        do {
            if (n->obj && h == n->hash && IDX_FN::equals(key, *n->obj)) {
                do {
                    f(*n->obj);
                } while ((n = n->nxt) && (h == n->hash) && IDX_FN::equals(key, *n->obj));
                return;
            }
        } while ((n = n->nxt));
    }
...
private:
    ...
    FORCE_INLINE void insertBST(T* obj, IdxEquivNode* & root) {
        ...
        while (curr != nullptr) {
            ...
            if (IDX_FN2::equals(*obj, *currObj) == is_max ? 1 : -1) { // SBJ: It is the compare function
    ...
    FORCE_INLINE void removeBST(const T* obj, IdxEquivNode* & root) {
        ...
        while (curr != nullptr) {
            ...
            if (IDX_FN2::equals(*currObj, *obj) == is_max ? 1 : -1) { //SBJ: Compare function
                curr = curr->right;
...
public:
    ...
    TreeIndex(size_t size = DEFAULT_CHUNK_SIZE, double load_factor = .75) : equiv_nodes_(size), nodes_(size / 10) {
        load_factor_ = load_factor;
        size_ = 0;
        count_ = 0;
        buckets_ = nullptr;
    ...
    FORCE_INLINE virtual void add(T* obj, const HASH_RES_t h) {
        ...
        do {
            if (h == n->hash && n->equivNodes && IDX_FN1::equals(*obj, *n->equivNodes->obj)) {
    ...
    FORCE_INLINE virtual void del(const T* obj, const HASH_RES_t h) override{
        ...
        do {
            next = n->nxt;
            if (n->hash == h && n->equivNodes && IDX_FN1::equals(*obj, *n->equivNodes->obj)) {
    ...
    void printTree(const T& key) {
        ...
        do {
            if (n->equivNodes && h == n->hash && IDX_FN1::equals(key, *n->equivNodes->obj)) {
...
class ArrayIndex : public Index<T, V>{
    ...
    FORCE_INLINE virtual T* get(const T& key) const {
    ...
    FORCE_INLINE virtual void foreach(std::function<void (const T&) > f) const {
