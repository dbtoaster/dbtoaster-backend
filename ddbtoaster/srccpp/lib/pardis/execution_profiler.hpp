#ifndef DBTOASTER_PARDIS_EXECUTIONPROFILER_HPP
#define DBTOASTER_PARDIS_EXECUTIONPROFILER_HPP
...
struct ExecutionProfiler {
  ...
  static void printProfile() {
    ...
    for (auto it : durations) {
      ...
      std::cout << it.first << "   count = " << counters[it.first] 
                << " time = " << it.second / 1000000.0 << " ms    avg time = " 
                << (c ? it.second / c : 0) << " ns" << std::endl;
    }
#else
  ...
  static void printProfileToFile(const std::string& name = "profile.csv") {
    ...
    for (auto it : durations) {
      size_t c = counters[it.first];
      fout << it.first << "," << counters[it.first] << "," 
           << it.second / 1000000.0 << "," << (c ? it.second / c : 0) << std::endl;
    }
    fout.close();
#endif
  }

  static void printProfileToFilePartitioned(int partId, std::unordered_map<std::string, size_t>& DUR, 
                                            std::unordered_map<std::string, size_t>& COU,
                                            const std::string& name = "profile.csv") {
#ifdef EXEC_PROFILE
    std::ofstream fout(name, std::ios::app);
    if (partId == 0)
      fout << ",count,timeMS,avgTimeNS" << std::endl;
    ...
    for (auto it : DUR) {
      ...
      fout << "part-" << partId << ":" << it.first << "," << COU[it.first] << "," 
           << it.second / 1000000.0 << "," << (c ? it.second / c : 0) << std::endl;
    }
    fout.close();
#endif
...
#endif /* DBTOASTER_PARDIS_EXECUTIONPROFILER_HPP */