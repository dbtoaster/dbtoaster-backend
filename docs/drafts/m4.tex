...
Note: we could be tempted to lazily devise workers participating to the barrier to reduce the number of messages exchanged and possible stragglers. The issues with this approach is that the master does not know the set of indirectly affected workers, thereby requiring acknowledgements to discover them, thus latency would be increased. Additionally, this complicates implementation as \textit{barrier-start} needs to be piggybacked on potentially all calls.
