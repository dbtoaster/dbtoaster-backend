...
\section*{Overview}
\begin{center}
\setlength{\unitlength}{.4cm} \begin{picture}(30,19)\footnotesize %\put(0,0){\tbox{30}{19}{}}
\put(0,13) { % Front-end
	\put(0,4){\tbox[0l]{5}{1.4}{\bf SQL}}
	\put(0,2){\tbox[0l]{5}{1.4}{\bf M4 calculus}}
	\put(0,0){\tbox[0l]{5}{1.4}{\bf Scala DSL}}
	\put(6,4){\tbox{4.5}{1.4}{SQL parser}}
	\put(6,2){\tbox{4.5}{1.4}{M4 parser}}
	\put(6,0){\tbox{4.5}{1.4}{Lifting}}
	\put(12,4){\tbox{4.2}{1.4}{SQL to calc}}
	\put(12,2){\tbox[0]{4.2}{1.4}{M4 calc IR}}
	\put(18,4){\tbox{7}{1.4}{Type Checking}}
	\put(18,1.5){\tbox{7}{1.4}{Incrementalization}}
	\put(18,-1){\tbox{7}{1.4}{Distribution}}
	\put(18,-3.2){\tbox[0]{7}{1.4}{M4 triggers IR}}
	{\linethickness{1pt}
...
\put(0,0){ % Back-end
	\put(7.8,7){\tbox{5}{1.4}{ScalaGen}}
	\put(13.4,7){\tbox{5}{1.4}{AkkaGen}}
	\put(19,7){\tbox{5}{1.4}{LMS lifting}}
	\put(19,4.9){\tbox[0]{5}{1.4}{LMS graph IR}}
	\put(16.2,2){\tbox{5}{1.4}{LMS Scala}}
	\put(16.2,0){\tbox{5}{1.4}{LMS Akka}}
	\put(21.8,2){\tbox{5}{1.4}{Boost C++}}
	\put(21.8,0){\tbox{5}{1.4}{Cluster C++}}
	{\linethickness{1pt}
...
\put(0,0){ \put(26,4.9){\tbox[0]{4}{1.4}{\it Statistics}} % Statistics
...
M4, the upcoming language, is in a design phase. We plan to maintain the same M4 language across compilation phases to allow manual optimizations. In M4/calc it is possible to assign declaratively map content. In M4/triggers, all these assignments must be replaced by explicit triggers.

Front-ends:\ul
\item SQL: for users familiar with DBMS
\item M4 calculus: allows declarative recursion, triggers and distribution specifications
\item Scala DSL: has the same power as M4 calculus but can interact with Scala functions
\ule

Back-ends:\ul
\item ScalaGen: vanilla Scala generator (unoptimized)
\item AkkaGen: distributed (Akka) code generator
\item LMS Scala/Akka: LMS-optimized code generators for JVM (default)
\item Boost/Cluster C++: counterpart version of LMS targeting lower level C++
\ule

Runtime: (long-term goal) collects statistics to optimize triggers according to map loads at runtime. The runtime engine needs to bundle the code generator and compiler.
...
%First key part is the grouping columns, second part is aggregation values for the group, multiplicity is the number of tuples in the group. Aggregation values semantics is <<element>> for write and <<aggregate>> for read.
...
% Finally, if a <<filter>> keyword precedes the aggregation key, tuples that do not have the same aggregation value will be discarded.
...
{\bf Map options}\\
A map can declare the following options:\ul
\item \textbf{Tags:} a tag is a group identifier indicating which group of (cluster) nodes will be in charge of the map partitions. We can declare more than one tag to achieve redundancy. Hosts will then join the cluster with one or more tag, thereby identifying the map they are willing to be responsible of.
	\ule\discuss{Is the number of nodes variable? What if nodes are leaving/joining (failure/recovery) the system?There are two approaches: static hashing with redundancy (that is read/write from multiple nodes or \href{http://lpd.epfl.ch/alistarh/DistAlgoFall08/da08-GroupMembershipVSC.pdf}{group membership}. Is there an existing framework we can build on? Note that we could tune manually the location with hashing function.}\ul
\item \textbf{Threshold:} minimal value relative difference for a tuple update to be considered for recursion. This apply only to maps with value type {\tt double}.
	\ule\discuss{Do we also need this feature on {\tt double} keys ?}\ul
\item \textbf{Local:} a map being declared local will only be accessible from the local host (cluster). This helps encoding and enforcing local operations as in \cite{webdamlog}.
\item \textbf{GPU:} whether the map is to be stored and processed in parallel with CUDA on GPU (when the node hardware supports it). This is planned as a long-term optimization.
\ule\discuss{Rationale: it is possible to have decent hash maps on GPUs \cite{gpu_hashing}, bottleneck is CPU (memory bandwidth?) and program consists of loops that often execute the same task (smaller divergence).
\say{Thierry}{At least we should benchmark GPU maps vs CPU maps before we discard this idea.}}\ul
...
\item Call: workers and master maintain locally the number of remote calls started remotely on each worker in a map $C_S\langle worker\to\#calls \rangle$. Additionally, workers maintain locally the number of calls $C_R$ they have received. Note: the \textit{barrier-start} message counts as a remote call.
\ule
...
Note 1: we could be tempted to lazily devise workers participating to the barrier to reduce the number of messages exchanged and possible stragglers. The issues with this approach is that the master does not know the set of indirectly affected workers, thereby requiring acknowledgements to discover them, thus latency would be increased. Additionally, this complicates implementation as \textit{barrier-start} needs to be piggybacked on potentially all calls.

Note 2: if request/response cached locally at workers, it is necessary to disable the caching during the barrier phase procedure in order not to propagate stale information across barrier. Intuitions:\ul
\item Necessary: if clear happen only at barrier start, a call from peer might load a dirty value in the cache, if it happens at barrier end, the end message might be delayed arbitrarily
\item Sufficient: assume there is a stale value in cache, this means that the node will block the barrier, hence will clean its cache before crossing the barrier.
\ule
...
\discuss{Not formally proved, needs review/verification}
Modifying the group membership asynchronously avoids blocking the streams during the reshuffling (which might be costly depending the database size) but delays the departures until the node has no more valid data.\ol
\item After a barrier, the master broadcasts the membership list to all workers and install it locally. Then it continues execution.
\item When a worker receives the new list, it maintains both the old and new membership lists. We denote by $w_n$ the worker that is responsible of a map partition in the new membership list and $w_o$ the worker that was previously responsible of that partition. Intuitively $w_n$ is responsible (authority) of the partition but has a fallback on $w_o$: \ul
	\item On {\tt get(k)} at $w_n$: if the key is not present, $w_n$ {\tt get+del(k)} $w_o$ and stores the result locally. Then it process the original request normally.
	\item On {\tt add(k)} at $w_n$: if the key is not present, $w_n$ {\tt get+del(k)} $w_o$ before processing locally.
	\item On {\tt set(k)} at $w_n$: $w_n$ sets value locally and sends {\tt del(k)} to $w_o$ if the key was not present.
	\item On {\tt clear} at $w_n$: $w_n$ can detach from $w_o$ for this particular map an is fully authoritative.
	\item On {\tt foreach} and {\tt aggr} at $w_n$: we must ensure that the same computation does not happen twice (i.e. a key is not present in both $w_n$ and $w_o$. To do that, notice that if channels have FIFO property, all previous {\tt del} messages will be received on $w_o$ before this message, hence it is possible to execute on both hosts the closure and possibly re-aggregate at $w_n$ when response is received.
	\ule
\item The main problem of this scheme is that the map will tend to remain on $w_o$, hence $w_o$ needs to actively transfer it to $w_n$ to be able to reap memory / leave the group. To do that, $w_o$ sends {\tt optset(k,v)} messages to $w_n$. At reception on $w_n$, if the key is not present it is set and a {\tt del(k)} acknowledgement is sent back to $w_o$. Finally, when no key is present in $w_o$ for the partition it needs to make $w_n$ aware that it is fully authoritative.
\item When $w_n$ is fully authoritative, it can allow $w_o$ to free resources associated with the partition and possibly leave the group.
\ole
