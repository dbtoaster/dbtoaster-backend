...
Major novelty: each expression contains two columns: a value $v$ exposed to the user and an implicit multiplicity $m$ that can be understood as the SQL {\tt count(*)} operator in every expression (i.e. how many tuples are consumed to construct $v$). The semantics is:
