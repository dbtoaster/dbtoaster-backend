...
\def\discuss#1{\par\hspace{2em}\begin{minipage}{15cm}\footnotesize {\bf\color{red} Discussion:} #1\end{minipage}\par}
...
\say{Milos}{disagree with this semantics, think that the resulting multiplicity of aggregation should be 1.}
}

% --------------------------------------------------------------------------------------------------------------
\subsubsection*{Queries (output)}
\begin{verbatim}
query ::= "DECLARE" "QUERY" name ":=" mapName ";"
\end{verbatim}
...
\begin{verbatim}
a couple of suggestions/questions that i have come up with so far. let me know if you are interested to discuss them:
- User defined triggers might help with input from the outside (from non-files -> user interaction), alternative: 
user defined streams -> define an interface for streams
==> flexibility in the streams is important
==> define a class as input/output stream (output necessary ?)

- What about different map types/indexes? should the user be able to define them?
==> map as hash map, map as tree, how to give a hint to the compiler ?
==> secondary indices as trees (index on a key part)
==> inequality join?
==> make an option and remove once we can automate generation

- Instead of declaring queries seperately, we could just declare maps to be PUBLIC (DEFINE PUBLIC MAP)
==> good ! Let's have it !!

- I would suggest to get rid of ON SYSTEM READY and have a stream of system events instead
==> not a problem. reduce tables to only what necessary ?

- What does it mean for a map to have a location?
==> variable number of nodes, have a specific hashing

- Why do we need to specify a type for an aggregate explicitely?
==> Should be able to define it ==> make it optional

Have varying number of nodes leaving/joining the system ?
==> How to change the locality of maps ?
==> Redundancy with multiple hashes ==> options of redundancy
