...
The major novelty is that each table/map/expression contains two columns: the value and a special multiplicity $m$ can be understood as the SQL {\tt count(*)} operator applied to every expression or \textit{how many tuples are used to construct the expression}. The semantics is:
...
\erule[]{v_1 \odot v_2}{r,1}{r\in\{0,1\}\text{ the result of }v_1 \odot v_2}
\end{array}\]
