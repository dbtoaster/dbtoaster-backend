...
trait K4Map[K,A,V] {
  def get(key:K) : (V,A)
  def set(key:K, value:V, agg:A=null)
  def add(key:K, value:V, agg:A=null)
  def foreach(f:(K,V,A)=>Unit)
  def foreach(f:(K,V)=>Unit)
  def agg[R](f:(K,V,A)=>R)(implicit cR:ClassTag[R]) : R
  def agg[R](f:(K,V)=>R)(implicit cR:ClassTag[R]) : R
  
  def slice[P](part:Int, partKey:P) : K4Map[K,A,V]
  def clear()
  def size : Int
  def toMap : Map[K,(V,A)]
  def toString : String
