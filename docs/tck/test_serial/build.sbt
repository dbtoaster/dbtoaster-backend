...
val cc = (sourceDirectory,classDirectory in Compile) map { (src,dest) =>
  Seq("gcc","-O3","-shared","-I"+src+"/include",src+"/serial.c","-o",dest+"/serial/NodeIOImpl.jnilib").!
}

Seq(
  compile <<= (compile in Compile) dependsOn cc,
  run <<= (run in Compile) dependsOn cc
)
/*
