...
trait ScalaGenM3Ops extends ScalaGenBase with ScalaGenEffect {
  ...
  val names = new java.util.HashMap[Int,String]()

  import ddbt.Utils.tup
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case Named(n) => names.put(sym.id,n)
    //case Named(n) => emitValDef(sym, n)
  ...
  override def quote(x: Exp[Any]) : String = x match {
    case Sym(id) if names.containsKey(id) => names.get(id)
