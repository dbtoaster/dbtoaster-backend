 ...
 */
trait K3MapGenerator extends GeneratorOps {
  type MapType[K,V] = K3Map[K,V]

  implicit def k3Map2Generator[K:Manifest,V:Manifest](x: Rep[K3Map[K, V]]): TupleGenerator[K,V] = k3ToGenerator(x)

  def newK3Map[K: Manifest, V: Manifest](name: Rep[String], elems: Rep[Map[K, V]], sndIdx: Rep[Option[Map[String, Index[K, V]]]]): Rep[K3Map[K, V]]
  def k3Lookup[K, V: Manifest](x: Rep[K3Map[K, V]], key: Rep[K]): Rep[V]
  ...
  def k3Foreach[K: Manifest, V: Manifest](x: Rep[K3Map[K, V]], fn: => Rep[Tuple2[K, V]] => Rep[Unit]): Rep[Unit]
  def k3ToGenerator[K:Manifest,V:Manifest](x: Rep[K3Map[K, V]]) : TupleGenerator[K,V] = new TupleGenerator[K,V]{
...
/**
 * Lifter Classes for K3Map
 */
trait K3MapOps extends Variables with K3MapGenerator{

  class K3MapOpsCls[K: Manifest, V: Manifest](x: Rep[K3Map[K, V]]) {
    ...
    def mutable: Rep[K3Map[K, V]] = k3Mutable[K,V](x)
    //def foreach(fn: Rep[Tuple2[K, V]] => Rep[Unit]): Rep[Unit] = k3Foreach[K, V](x, fn)
  }

  implicit def k3Map2K3MapOpsCls[K: Manifest, V: Manifest](x: Rep[K3Map[K, V]]): K3MapOpsCls[K, V] = new K3MapOpsCls[K, V](x)

  def k3Contains[K, V](x: Rep[K3Map[K, V]], key: Rep[K]): Rep[Boolean]
  def k3Remove[K, V](x: Rep[K3Map[K, V]], key: Rep[K]): Rep[Unit]
  def k3Slice[K:Manifest,V:Manifest,K2:Manifest](x: Rep[K3Map[K, V]], kp: Rep[K2], idx: Rep[List[Int]]): Rep[K3Map[K, V]]
  def k3Clear[K, V](x: Rep[K3Map[K, V]]): Rep[Unit]
  def k3Mutable[K: Manifest, V: Manifest](x: Rep[K3Map[K, V]]): Rep[K3Map[K, V]]

}

trait K3MapExp extends K3MapOps with GeneratorOpsExp with BaseExp with EffectExp with VariablesExp {

  case class NewK3Map[K, V](mK: Manifest[K], mV: Manifest[V], name: Exp[String], elems: Exp[Map[K, V]],
    sndIdx: Exp[Option[Map[String, Index[K, V]]]]) extends Def[K3Map[K, V]]
  case class K3Contains[K, V](x: Exp[K3Map[K, V]], key: Exp[K]) extends Def[Boolean]
  case class K3Lookup[K, V:Manifest](x: Exp[K3Map[K, V]], key: Exp[K]) extends Def[V] {
    val mV = manifest[V]
  }
  case class K3LookupOrDefault[K, V:Manifest](x: Exp[K3Map[K, V]], key: Exp[K], defaultVal: Exp[V]) extends Def[V] {
    val mV = manifest[V]
  }
  case class K3UpdateValue[K, V](x: Exp[K3Map[K, V]], key: Exp[K], value: Exp[V]) extends Def[Unit]
  case class K3Remove[K, V](x: Exp[K3Map[K, V]], key: Exp[K]) extends Def[Unit]
  case class K3Slice[K:Manifest,V:Manifest,K2:Manifest](x: Exp[K3Map[K, V]], kp: Exp[K2], idx: Exp[List[Int]]) extends Def[K3Map[K, V]] {
    val mK = manifest[K]
    val mV = manifest[V]
    val mK2 = manifest[K2]
  }
  case class K3Clear[K, V](x: Exp[K3Map[K, V]]) extends Def[Unit]
  case class K3Mutable[K, V](x: Exp[K3Map[K, V]]) extends Def[K3Map[K, V]]
  case class K3Foreach[K, V](x: Exp[K3Map[K, V]], v: Sym[(K,V)], block: Block[Unit]) extends Def[Unit]

  def newK3Map[K: Manifest, V: Manifest](name: Exp[String], elems: Exp[Map[K, V]],
    sndIdx: Exp[Option[Map[String, Index[K, V]]]]): Exp[K3Map[K, V]] = reflectMutable(NewK3Map(manifest[K], manifest[V], name, elems, sndIdx));

  def k3Contains[K, V](x: Exp[K3Map[K, V]], key: Exp[K]) = K3Contains(x, key)
  def k3Lookup[K, V: Manifest](x: Exp[K3Map[K, V]], key: Exp[K]) = K3Lookup(x, key)
  def k3LookupOrDefault[K, V: Manifest](x: Exp[K3Map[K, V]], key: Exp[K], defaultVal: Exp[V]) = K3LookupOrDefault(x, key, defaultVal)
  def k3UpdateValue[K, V](x: Exp[K3Map[K, V]], key: Exp[K], value: Exp[V]) = reflectWrite(x)(K3UpdateValue(x, key, value));
  
  def newMapType[K: Manifest, V: Manifest]() = newK3Map[K,V](unit("NEW_K3_MAP"), unit(Map()), unit(None))
  def lookupOrDefault[K, V: Manifest](x: Exp[K3Map[K, V]], key: Exp[K], defaultVal: Exp[V]) = k3LookupOrDefault(x, key, defaultVal)
  def updateValue[K, V](x: Exp[K3Map[K, V]], key: Exp[K], value: Exp[V]) = k3UpdateValue(x, key, value)

  def k3Remove[K, V](x: Exp[K3Map[K, V]], key: Exp[K]) = reflectWrite(x)(K3Remove(x, key))
  def k3Slice[K:Manifest,V:Manifest,K2:Manifest](x: Exp[K3Map[K, V]], kp: Exp[K2], idx: Exp[List[Int]]) = K3Slice[K,V,K2](x,kp,idx)
  def k3Mutable[K: Manifest, V: Manifest](x: Exp[K3Map[K, V]]) = reflectMutable(K3Mutable[K,V](x))
  def k3Clear[K, V](x: Exp[K3Map[K, V]]): Exp[Unit] = reflectWrite(x)(K3Clear(x))
  def k3Foreach[K: Manifest, V: Manifest](x: Exp[K3Map[K, V]], fn: => Exp[Tuple2[K, V]] => Exp[Unit])  = {
    val v = fresh[(K,V)]
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case e@NewK3Map(mK, mV, name, elems, sndIdx) => newK3Map(f(name), f(elems), f(sndIdx))(e.mK, e.mV)
    case K3Contains(x, key) => k3Contains(f(x), f(key))
    case e@K3Lookup(x, key) => k3Lookup(f(x), f(key))(mtype(e.mV))
    case e@K3LookupOrDefault(x, key, defaultVal) => k3LookupOrDefault(f(x).asInstanceOf[Exp[K3Map[Any, A]]], f(key), f(defaultVal))(mtype(e.mV)) //why cast is needed?
    ...
    case Reflect(e@NewK3Map(mK, mV, name, elems, sndIdx), u, es) => reflectMirrored(Reflect(NewK3Map(mtype(mK), mtype(mV), f(name), f(elems), f(sndIdx)), mapOver(f,u), f(es)))(mtype(manifest[A]))
    ...
    case Reflect(e@K3LookupOrDefault(x, key, defaultVal), u, es) => reflectMirrored(Reflect(K3LookupOrDefault(f(x).asInstanceOf[Exp[K3Map[Any, A]]], f(key), f(defaultVal))(mtype(e.mV)), mapOver(f,u), f(es)))(mtype(manifest[A]))
...
trait ScalaGenK3Map extends ScalaGenBase with GenericNestedCodegen {
  val IR: K3MapExp
  import IR._

  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NewK3Map(mK, mV, name, elems, sndIdx) => emitValDef(sym, "new K3Map[" + remap(mK) + "," + remap(mV) + "](" + quote(name) + ",new HashMap[" + remap(mK) + "," + remap(mV) + "]," + quote(sndIdx) + ")")
    ...
    case K3Mutable(x) => emitValDef(sym, "" + quote(x) + " // mutable K3Map");
...
trait K3MapExpOpt extends BaseExp with NestedGraphTraversal with K3MapExp with IfThenElseExp with TupleOpsExp { self =>
  ...
  override def k3Foreach[K: Manifest, V: Manifest](x: Exp[K3Map[K, V]], fn: => Exp[Tuple2[K, V]] => Exp[Unit]) = {
    val v = fresh[(K,V)]
