...
object Load{
  ...
  def loadItems(loadConfig: TpccLoadConfig, option_debug: Boolean) {
    ...
    while (i_id <= MAXITEMS) {
      ...
      i_price = ((Util.randomNumber(100, 10000)).toInt / 100.0).toDouble
  ...
  def district(loadConfig: TpccLoadConfig, w_id: Int): Boolean = {
    ...
    var d_tax: Double = 0.0
    var d_ytd: Double = 0.0
    var d_next_o_id: Int = 0
    val error = false
    System.out.print("Loading District\n")
    d_w_id = w_id
    d_ytd = 30000.0.toDouble
    d_next_o_id = 3001
    ...
    while (d_id <= DIST_PER_WARE) {
      ...
      d_tax = (Util.randomNumber(10, 20).toDouble / 100.0).toDouble
  ...
  def loadCustomer(loadConfig: TpccLoadConfig, 
    ...
    if ((currentShard == shardId) || (shardId == 0)) {
      ...
      while (c_id <= CUST_PER_DIST) {
        ...
        c_credit += "C"
        c_credit_lim = 50000
        c_discount = (Util.randomNumber(0, 50).toDouble / 100.0).toDouble
        c_balance = -10.0.toDouble
  ...
  def loadOrders(loadConfig: TpccLoadConfig, 
    ...
    var ol_amount: Double = 0.0
    var ol_dist_info: String = null
    var tmp_Double: Double = 0.0
    var currentShard = 0
    if (shardCount > 0) {
      currentShard = (w_id % shardCount)
      if (currentShard == 0) {
    ...
    if ((currentShard == shardId) || (shardId == 0)) {
      ...
      while (o_id <= ORD_PER_DIST) {
        ...
        while (ol <= o_ol_cnt) {
          ...
          ol_quantity = 5
          ol_amount = 0.0.toDouble
          ol_dist_info = Util.makeAlphaString(24, 24)
          tmp_Double = ((Util.randomNumber(10, 10000)).toDouble / 100.0).toDouble
          if (o_id > 2100) {
            ...
            orderLineRecord.add(ol_quantity)
            orderLineRecord.add(tmp_Double)
            orderLineRecord.add(ol_dist_info)
            orderLineLoader.load(orderLineRecord)
          }
          if (optionDebug) {
