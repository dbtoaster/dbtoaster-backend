...
class Payment(var pStmts: TpccStatements) extends IPayment {
  ...
  override def paymentTx(currentTimeStamp:Date, t_num: Int, 
    ...
    try {
      ...
      proceed = 1
      try {
        pStmts.getStatement(9).setDouble(1, h_amount)
      ...
      proceed = 3
      try {
        pStmts.getStatement(11).setDouble(1, h_amount)
      ...
      try {
        ...
        pStmts.getStatement(19).setDouble(7, h_amount)
