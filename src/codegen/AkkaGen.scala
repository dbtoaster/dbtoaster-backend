...
class AkkaGen(cls:String="Query") extends ScalaGen(cls) {
  ...
  private var stmt_n=0; // # of sequential statements to close
  ...
  def remote(m:String,fn:String,f:()=>String):(String,List[String]) = {
    local_r=m; val c=ctx.save++ctx.ctx0; val bk=dep.in(false); val body=f(); local_r=null; dep.out(bk)
  ...
  override def genStmt(s:Stmt) = s match {
    case StmtMap(m,e,op,oi) => val r=ref(m.name); val (fop,sop)=op match { case OpAdd => ("add","+=") case OpSet => ("set","=") }
      ...
      def pre(o:OpMap,e:Expr,clear:Boolean=true):String = { // preparation (conditional barrier) for map operation
        (if (o==OpSet && m.keys.size>0 && clear) { stmt_n+=1; "pre("+r+",true,Array(),()=> {\nclear("+r+");\n" } else "")+
        ({ stmt_n+=1; "pre("+r+","+(if (o==OpSet) "true" else "false" )+",Array("+rd(e).mkString(",")+"),()=>{\n" })
      }
      ...
      val init = oi match { case None => "" case Some(ie) => ctx.load(); inuse.load(m.keys.toSet)
        ...
        dep.rst; pre(OpSet,ie,false)+ind(cpsExpr(ie,(v:String)=>dep.cl(()=>co(v))))+"\n"
      }
      ctx.load(); inuse.load(m.keys.toSet); dep.rst; init+pre(op,e)+ind(cpsExpr(e,(v:String)=>dep.cl(()=>mop(fop,v))))+"\n"
  ...
  override def genTrigger(t:Trigger):String = { // add pre and deq/ready calls in master's triggers
    ...
    ctx=Ctx(as.toMap); stmt_n=0; val body=t.stmts.map(genStmt).mkString; ctx=null; val close=(0 until stmt_n).map(_=>"})").mkString(" ")+(if (stmt_n>0) "\n" else "")
