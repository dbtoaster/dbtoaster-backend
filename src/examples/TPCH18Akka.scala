package examples
import ddbt.lib.{K3Map,K4Map,Helper,K3Helper}
...
class TPCH18AkkaQuery() extends DBTQuery {
  ...
  val sys = Helper.system
  val N = 16

  val QUERY18 = K4Map.make[(String,Long,Long,Date,Double), Double](sys,N)
  val mORDERS2 = K4Map.make[(String,Long), Long](sys,N,List((x:(String,Long))=>x._2)) // 0=List(1)
  val mCUSTOMER1 = K4Map.make[(Long,Long,Date,Double), Double](sys,N,List((x:(Long,Long,Date,Double))=>x._2)) // 0=List(1)
  val mCUSTOMER1_mLINEITEM1 = K4Map.make[(Long,Long,Date,Double), Long](sys,N,List(
    (x:(Long,Long,Date,Double))=>x._1, // 0=List(0)
    (x:(Long,Long,Date,Double))=>x._2  // 1=List(1)
  ))
  val mLINEITEM1 = K4Map.make[(Long,String,Long,Date,Double), Double](sys,N)
  val mLINEITEM1_mLINEITEM1 = K4Map.make[(Long,String,Long,Date,Double), Long](sys,N,List((x:(Long,String,Long,Date,Double))=>x._1)) // 0=List(0)
  val mLINEITEM1_E1_1_L1_1 = K4Map.make[Long,Double](sys,N)
  ...
  def act(): Unit ={
    ...
    while(true) {
      receive {
        case EndOfStream => {
          println(K3Helper.toStr(getQUERY18().toMap))
          Helper.system.shutdown
          //printResults()
          supervisor ! DBTDone;
          exit();
        }
  ...
  def printResults(): Unit = {
    ...
    println(pp.format(<QUERY18>{ K3Helper.toXML(getQUERY18().toMap) }</QUERY18>));
  }
  def printMapSizes() = {
/*
    println(NATION.size)
