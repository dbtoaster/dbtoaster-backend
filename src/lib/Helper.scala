...
object Helper {
  ...
  def extractExecArgs(args:Array[String]) = {
    ...
    val no_output = args.exists(_ == "--no_output")
    (num, mode, timeout, parallel, ds, batchSize, no_output)
  }
  def bench(args:Array[String],run:(String,Int,Long,Int,Boolean)=>(StreamStat,List[Any]),op:List[Any]=>Unit=null) {
    val (num, mode, timeout, parallel, ds, batchSize, no_output) = extractExecArgs(args)
    ...
    ds.foreach { d=> var i=0; var res0:List[Any]=null
      while (i < num) { i+=1;
        val (t,res)=run(d,parallel,timeout,batchSize,no_output);
        if (t.skip==0) {
          if (res0==null)
            res0=res
          else
