...
object Utils {
  ...
  def cppCompiler(out:String,cPath:String,boost:String,cppLibDir:String) = {
    val as = (List(prop("gpp","g++"),cppLibDir+"/main.cpp","-Wall","-Wno-unused-variable","-Wno-unused-but-set-variable","-Wno-strict-overflow","-std=c++11","-include",out,"-o",cPath,"-O3","-DNDEBUG","-lpthread","-ldbtoaster", "-ljemalloc","-I"+cppLibDir,"-L"+cppLibDir) :::
