...
object AllQueries {
  ...
  import ddbt.UnitTest.{tmp,all,genQueryScala,Printer,QueryTest}
  ...
  def genTest(q:String,repo:Boolean) {
    ...
    val m3 = try {
      ...
      UnitTest.benchmark=run; genQueryScala("Query",QueryTest(q),new Printer("Scala"),m3,ddbt.Compiler.LANG_SCALA,false)
  ...
  def zeus(s:Long=0) {
    val sql = exec("scripts/zeus.rb"+(if (s!=0) " -s "+s else ""),true)._1.replaceAll("@@DATA@@",path_repo+"/../../experiments/data/simple/tiny")
