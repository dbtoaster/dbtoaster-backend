...
class Q18Worker extends WorkerActor {
  ...
  def forl(f:FunRef,args:Array[Any],co:Unit=>Unit) = (f,args) match {
    // Lineitem triggers
    case (`lf`,Array(ff:Int,l_okey:Long,l_qty:Double,mul:Int)) =>
      ff match {
        case 0 => mCUSTOMER1_L1.slice(0,l_okey).foreach { case (k,v) => add(mC,(l_okey,k._2,k._3,k._4), v * l_qty * mul) };
        case 1 => mLINEITEM1_L1.slice(0,l_okey).foreach { case (k,v) => add(mL,(l_okey,k._2,k._3,k._4,k._5), v * l_qty * mul) };
      }
      co()
    case (`lf2`,_) =>
      mLINEITEM1.foreach { case (k,v) => val o_key:Long=k._1; get(mLE,o_key,(l3_qty:Double) => set(mQ18,(k._2,k._3,o_key,k._4,k._5), v * (if (100L<l3_qty && l3_qty!=0L) 1L else 0L) )) }; 
      co()
    // Orders triggers
    case (`of`,Array(ff:Int,l3_qty:Double,c_key:Long,o_key:Long,o_date:Date,o_total:Double,mul:Int)) => 
      ff match {
        case 2 => mORDERS2.slice(0,c_key).foreach { case (k,v) => add(mQ18,(k._1,c_key,o_key,o_date,o_total), (v * (if (100L<l3_qty && l3_qty!=0L) 1L else 0L) * l3_qty * mul)) }
        case 3 => mORDERS2.slice(0,c_key).foreach { case (k,v) => add(mL,(o_key,k._1,c_key,o_date,o_total), v * l3_qty) }
        case 4 => mORDERS2.slice(0,c_key).foreach { case (k,v) => add(mLL,(o_key,k._1,c_key,o_date,o_total), v) }
      }
      co()
    // Customer triggers
    case (`cf`,Array(ff:Int,c_key:Long,c_name:String,mul:Int)) =>
      ff match {
        case 5 => mCUSTOMER1.slice(0,c_key).foreach { case (k,v) => val o_key:Long=k._1; get(mLE,o_key,(l3_qty:Double)=>{ add(mQ18,(c_name,c_key,o_key,k._3,k._4), v * (if (100L<l3_qty && l3_qty!=0L) 1L else 0L) * mul) }) }
        case 6 => mCUSTOMER1.slice(0,c_key).foreach { case (k,v) => add(mL,(k._1,c_name,c_key,k._3,k._4), v * mul) }
        case 7 => mCUSTOMER1_L1.slice(1,c_key).foreach { case (k,v) => add(mLL,(k._1,c_name,c_key,k._3,k._4), v * mul) }
      }
      co()
    // Errors
    case _ => sys.error("Unmatched message "+f+" + "+args.toList)
  }
...
class Q18Master extends Q18Worker with MasterActor {
  ...
  def onAddLINEITEM(l_okey:Long,l_pkey:Long,l_skey:Long,l_linenum:Long,l_qty:Double,l_extprice:Double,l_discount:Double,l_tax:Double,l_retflag:String,l_status:String,
    ...
    pre(mC,mCL); foreach(mCL,lf,0,l_okey,l_qty,1)
    pre(mL,mLL); foreach(mLL,lf,1,l_okey,l_qty,1)
    pre(mLE); add(mLE,l_okey, l_qty)
    clear(mQ18); barrier; pre(mQ18,mL); foreach(mL,lf2)
    deq
  }
  ...
  def onAddORDERS(o_key:Long,c_key:Long,O_ORDERSTATUS:String,o_total:Double,o_date:Date,o_pri:String,o_clerk:String,o_shippri:Long,o_comm:String) = reset {
    ...
    pre(mQ18,mO); if (l3_qty!=0.0) { foreach(mO,of,2,l3_qty,c_key,o_key,o_date,o_total,1) }
    pre(mC); add(mC,(o_key,c_key,o_date,o_total), l3_qty)
    pre(mCL); add(mCL,(o_key,c_key,o_date,o_total), 1L)
    pre(mL,mO); if (l3_qty!=0.0) { foreach(mO,of,3,l3_qty,c_key,o_key,o_date,o_total,1) }
    pre(mLL,mO); foreach(mO,of,4,l3_qty,c_key,o_key,o_date,o_total,1)
    deq
  }
  ...
  def onAddCUSTOMER(c_key:Long,c_name:String,c_addr:String,c_nat:Long,c_phone:String,c_acctbal:Double,c_mktseg:String,c_comm:String) = reset {
    pre(mQ18,mC,mLE); foreach(mC,cf,5,c_key,c_name,1)
    add(mO,(c_name,c_key), 1L)
    pre(mL,mC); foreach(mC,cf,6,c_key,c_name,1)
    pre(mLL,mCL); foreach(mCL,cf,7,c_key,c_name,1)
    deq
  }
